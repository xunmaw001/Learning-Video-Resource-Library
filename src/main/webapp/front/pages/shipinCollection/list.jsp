                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="shipinUuidNumber" id="shipinUuidNumber" placeholder="视频编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="shipinName" id="shipinName" placeholder="视频名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div class="dataStyle">
                        <figure v-for="(item,index) in dataList" v-bind:key="index" class="effect-bubba">
                            <i @click="deleteShipinCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                            <img :src="item.shipinPhoto?item.shipinPhoto.split(',')[0]:''"  style="height: 350px;width:100%;" />
                            <figcaption>
                                <h2><span>{{item.shipinName}}</span></h2>
                                <p v-if="item.shipinNewMoney != null" >￥{{item.shipinNewMoney}}</p>
                                <a @click="jumpCheck('../shipin/detail.jsp?id='+item.shipinId , item.shipinDelete == null?'':item.shipinDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" >View more</a>
                            </figcaption>
                        </figure>
                    </div>
                    <nav class="pages">
...
<script>
    ...
    // 当前表的字典表
        ...
        function shipinCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shipin_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.shipinCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function pageList() {
            ...
            if (jquery('#shipinUuidNumber').val() != null && jquery('#shipinUuidNumber').val() != "" && jquery('#shipinUuidNumber').val() != "null" ) {
                param['shipinUuidNumber'] = jquery('#shipinUuidNumber').val();
            }
            ...
            //收藏只能查类型为1的
            param['shipinCollectionTypes'] = 1;

            // 获取列表数据
            http.request('shipinCollection/page', 'get', param, function(res) {
