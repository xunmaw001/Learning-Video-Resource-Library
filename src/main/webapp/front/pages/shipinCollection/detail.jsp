...
<body>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="blog-details-wrapper">
                    <div><img :src="detail.shipinCollectionPhoto" alt="image" style="width: 80%;height: auto" ></div>
                    <div class="blog-details-header">
                        <h3 class="blog-details-title">{{title}}</h3>
                        <ul class="post-meta">
                            <li v-if="detail.shipinCollectionTypes">类型：
                                {{detail.shipinCollectionValue}}
                            </li>
                        </ul>
        ...
        <div class="col-lg-4" v-if="shipinCollectionRecommendList">
            <div class="sidebar">
                ...
                <div class="widget">
                    ...
                    <ul class="small-post-list">
                        <li class="small-post-single" v-for="(item,index) in shipinCollectionRecommendList" v-bind:key="index" @click="jump('../shipinCollection/detail.jsp?id='+item.id)">
                            <div class="thumb"><img width="65px" :src="item.shipinCollectionPhoto"  alt="image"></div>
                            <div class="content">
                                <a class="main_color" class="date">{{item.shipinCollectionName}}</a>
                                <h6 v-if="item.shipinCollectionNewMoney" class="post-title"><a style="color: red">{{item.shipinCollectionNewMoney}}</a>RMB</h6>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            //预约
            ,shipinCollectionYuyue(){
                let _this = this;
                // localStorage.setItem('shipinCollectionId', _this.detail.id);
                // _this.jump("../shipinCollectionOrder/add.jsp");

                let data={
                    shipinCollectionId:_this.detail.id,
                    yonghuId:localStorage.getItem("userid"),
                    shipinCollectionOrderYesnoTypes:1,
                }
                // 提交数据
                layui.http.requestJson('shipinCollectionOrder/add', 'post', data, function (res) {
                    if(res.code == 0) {
                        layui.layer.msg('预约成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            _this.jump("../shipinCollectionOrder/list.jsp");
                        });
                    }else{
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'rate'], function () {
        ...
        http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
            // 详情信息
            vue.detail = res.data;
            vue.title = vue.detail.shipinCollectionName;
           // 轮播图片
        ...
        http.request(`shipinCollection/list`, 'get', {
            page: 1,
            limit: 5,
            shipinCollectionTypes: vue.detail.shipinCollectionTypes,




        }, function (res) {
            vue.shipinCollectionRecommendList = res.data.list;
        });
