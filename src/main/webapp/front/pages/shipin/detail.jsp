        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="blog-details-wrapper">
                    <div><img :src="detail.shipinPhoto" alt="image" style="width: 80%;height: auto" ></div>
                    <div class="blog-details-header">
                        <h3 class="blog-details-title">{{title}}</h3>
                        <ul class="post-meta">
                            <li v-if="detail.shipinUuidNumber">视频编号：
                                {{detail.shipinUuidNumber}}
                            </li>
                            <li v-if="detail.shipinTypes">视频类型：
                                {{detail.shipinValue}}
                            </li>
                            <li v-if="detail.shipinClicknum">热度：
                        ...
                        <ul class="post-meta">
                            <li>
<!-- 视频 -->
                                <video style="width: 730px;" :src="detail.shipinVideo" controls="controls" width="300px">
                                    您的浏览器不支持视频播放
                                </video>
                            </li>
                        </ul>
                        ...
                        <!--<div style="padding: 20px;border: 0px solid #f2f2f2;margin-top: 20px;display: flex">
                            <div class="num-picker">
                                <button @click="shipinYuyue()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                                    立即预约
                                </button>
                            </div>
                        </div>-->
                    ...
                    <form class="comment-form">
                        <div class="row">
                            <div class="col-lg-12 form-group">
                                <textarea name="comment-message" id="shipinLiuyanText" placeholder="留言内容"></textarea>
                            </div>
                            <div class="col-lg-12">
                                <button lay-submit lay-filter="shipinLiuyanText" class="cmn-btn-border">提交</button>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <div class="sidebar">
                ...
                <div class="widget">
                    ...
                    <ul class="small-post-list">
                        <li class="small-post-single" v-for="(item,index) in shipinRecommendList" v-bind:key="index" @click="jump('../shipin/detail.jsp?id='+item.id)">
                            <div class="thumb"><img width="65px" :src="item.shipinPhoto"  alt="image"></div>
                            <div class="content">
                                <a class="main_color" class="date">{{item.shipinName}}</a>
                                <h6 v-if="item.shipinNewMoney" class="post-title"><a style="color: red">{{item.shipinNewMoney}}</a>RMB</h6>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            ,shipinYuyue(){
                let _this = this;
                // localStorage.setItem('shipinId', _this.detail.id);
                // _this.jump("../shipinOrder/add.jsp");

                let data={
                    shipinId:_this.detail.id,
                    yonghuId:localStorage.getItem("userid"),
                    shipinOrderYesnoTypes:1,
                }
                // 提交数据
                layui.http.requestJson('shipinOrder/add', 'post', data, function (res) {
                    if(res.code == 0) {
                        layui.layer.msg('预约成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            _this.jump("../shipinOrder/list.jsp");
                        });
                    }else{
